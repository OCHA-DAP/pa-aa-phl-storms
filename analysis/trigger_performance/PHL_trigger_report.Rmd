...
# Table footnotes
trigger_table_footnote <- "The readiness and activation trigger can reach
their threshold and activate independently from one another. The full
framework activation is met only if both the readiness and activation triggers
are reached. (Unlike in other rapid-onset pilots, the readiness trigger MUST
be met in order for the activation trigger to go forward. If only one of the
two triggers is reached, no interventions will take place). The readiness
trigger performance was assessed by comparing recommended activations
with historical typhoon forecasts, and the activation trigger
by running the 510 model on ECMWF hindcasts."
#
# # Data providers
