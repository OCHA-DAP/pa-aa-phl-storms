...
This notebook is for getting return periods
from the number of damaged houses
...
df_impact = pd.read_csv(
    MAIN_DIR / IMPACT_FILENAME, parse_dates=["Year"], index_col="id"
)
...
```python
def get_events(df):
    return df.groupby("Year").max()[["Totally"]].reindex(new_index).fillna(0)
...
```python
def get_return_period_analytical(
    df_rp: pd.DataFrame,
    rp_var: str,
...
def get_return_period_empirical(
    df_rp: pd.DataFrame, rp_var: str
) -> pd.DataFrame:
    df_rp = df_rp.sort_values(by=rp_var, ascending=False)
    n = len(df_rp)
...
rp_analytical = get_return_period_analytical(
    df_events, rp_var="Totally", show_plot=True
)
rp_empirical = get_return_period_empirical(df_events, rp_var="Totally")
rp_analytical_limited = get_return_period_analytical(
    df_events_limited, rp_var="Totally", show_plot=True
)
rp_empirical_limited = get_return_period_empirical(
    df_events_limited, rp_var="Totally"
)
```
...
ax.plot(
    rp_empirical_limited["rp"],
    rp_empirical_limited["Totally"],
    "x",
    label=f"data, limited",
)
